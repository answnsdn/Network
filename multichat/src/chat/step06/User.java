...
public class User extends Thread{
	...
	public void ioWork() { //처음 접속했을 때 한 번만 실행
		try {
			...
			//클라이언트와 통신할 수 있는 스트림을 생성하고 nickname입장 문자열을 출력
	...
	//쓰레드로 처리할 작업 - 접속한 해당 클라이언트가 전송하는 메세지를 계속 받아서 서버창에 출력
	public void run() {
		while(true) {
			try {
				String msg = br.readLine();
				serverView.taclientlist.append(nickname+"이 전송한 메세지 : "+msg+"\n");
			}catch(IOException e){
				e.printStackTrace();
			}
		}
